...
// set up pointers
volatile uint32* timer 		= (uint32*)TIMER_0_BASE;
...
volatile uint8 sw0 = 0x00;		// sw0 value

void timer_isr(void *context){

	static uint8 led_val;

	*timer = 0;			// clear timer interrupt

	led_val = *leds;	// read current value of leds
	led_val++;			// increase led value
...
void key1_isr(void *context){
	...
	*(pb + 3) = 0;		// clear pushbutton interrupt
...
int main(){
	...
	*(pb + 2) = 0x00000002;		// enable interrupts on key1
	*(pb + 3) = 0x00000000;		// clear pushbutton interrupt
